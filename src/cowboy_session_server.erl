...
init(Config) ->
    ...
    timer:send_after(Expire * 1000, stop),
%%     {ok, Expire_TRef} = timer:exit_after(timer:seconds(Expire), expire),
    timer:send_after(10, set_expire),
    {ok, #state{
        sid = SID,
        expire = Expire,
%%         expire_tref = Expire_TRef,
        timeout = seconds_now() + Expire,
        storage = Storage
    }}.
...
%%     {ok, New_TRef} = timer:exit_after(timer:seconds(Expire), self(), Expire_TRef),
    {noreply, State#state{timeout = seconds_now() + Expire}};
...
handle_info(stop, #state{expire = Expire, timeout = Timeout} = State) ->
    case seconds_now() < Timeout of
        true ->
            timer:send_after(Expire * 1000, stop),
            {noreply, State};
        false ->
